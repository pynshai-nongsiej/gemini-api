...
def gen_composition_long(beats, srcs, comp_id, shot_dir, accent, seed=None):
...
const CHAPTER_STYLE = '{chapter_style}';
  ...
  if (cardStyle === 'editorial') {{
    return (
      <AbsoluteFill style={{{{ justifyContent: 'flex-end', opacity: Math.max(0, fade) }}}}>
        <AbsoluteFill style={{{{ background: 'linear-gradient(180deg, transparent 40%, rgba(6,8,12,0.85))' }}}} />
