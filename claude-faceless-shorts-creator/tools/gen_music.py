...
# Layered detuned sines + slow tremolo + gentle noise floor via ffmpeg lavfi:
# an evolving ambient bed good enough to duck under narration.
# ---------------------------------------------------------------------------
SYNTH_LICENSE = "Local ffmpeg synthesis (lavfi); no external service, free to use"
SYNTH_SOURCE = "local:ffmpeg-synth"


def generate_local(bed_id, length_ms):
    """Synthesize an ambient bed locally. Returns raw mp3 bytes."""
    import tempfile
    seconds = max(10.0, min(600.0, length_ms / 1000.0))
    ds = f"{seconds:.2f}"
    # slight per-bed tonal variation so the library beds don't all sound identical
    root = {"ambient-pad": 110.0, "focus-drone": 82.4, "warm-bed": 130.8}.get(bed_id, 98.0)
    expr = (f"0.20*sin(2*PI*{root}*t)"
            f"+0.14*sin(2*PI*{root * 1.5:.2f}*t)"
            f"+0.10*sin(2*PI*{root * 2.0:.2f}*t)"
            f"+0.07*sin(2*PI*{root * 2.51:.2f}*t)"
            f"+0.05*sin(2*PI*{root * 0.5:.2f}*t)")
    with tempfile.NamedTemporaryFile(suffix=".mp3", delete=False) as tmp:
        tmp_path = tmp.name
    try:
        r = subprocess.run(
            ["ffmpeg", "-y", "-hide_banner", "-v", "error",
             "-f", "lavfi", "-i", f"aevalsrc={expr}:d={ds}:s=44100",
             "-f", "lavfi", "-i", f"anoisesrc=d={ds}:c=pink:a=0.02",
             "-filter_complex",
             f"[0:a]tremolo=f=0.13:d=0.35,aecho=0.8:0.75:250|500:0.45|0.3[s];"
             f"[1:a]lowpass=f=500[n];[s][n]amix=inputs=2:normalize=0,"
             f"afade=t=in:ss=0:d=2,afade=t=out:st={max(0, seconds - 3):.2f}:d=3,volume=1.5[a]",
             "-map", "[a]", "-c:a", "libmp3lame", "-q:a", "3", tmp_path],
            stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
        if r.returncode != 0 or not os.path.exists(tmp_path) or os.path.getsize(tmp_path) == 0:
            raise RuntimeError(f"ffmpeg music synth failed for {bed_id}: {r.stdout[-300:]}")
...
def main():
    ...
    def entry(s, rel, path, length_ms, source=SOURCE, model_used=model, license_=LICENSE):
        ...
        return {
            ...
            "peak_dbfs": peak, "loudness_lufs": lufs, "source": source, "model": model_used,
            "force_instrumental": force_instrumental, "license": license_,
    ...
    if todo and not api_key:
        print("ELEVENLABS_API_KEY not set — falling back to LOCAL ffmpeg synthesis "
              "(offline ambient beds; neural music needs the key).")
    ...
    for s, rel, path, length_ms in todo:
        ...
        if api_key:
            ...
            audio = generate_local(s["id"], length_ms)
            with open(path, "wb") as f:
                f.write(audio)
            by_id[s["id"]] = entry(s, rel, path, length_ms, source=SYNTH_SOURCE,
